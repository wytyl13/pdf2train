...
class EmbeddingServer:
    ...
    async def run_embedding_task(self, request: EmbeddingRunRequest, background_tasks: BackgroundTasks):
        ...
        try:
            ...
            # 添加到后台任务队列
            background_tasks.add_task(self._safe_run_embedding, doc_id)
    ...
    async def _safe_run_embedding(self, doc_id: int):
        """
        [后台任务包装] 安全执行业务逻辑，防止异常中断服务
        """
        try:
            await self.embedding_service.run_embedding_for_doc(doc_id)
        except Exception as e:
