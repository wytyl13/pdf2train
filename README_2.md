...
5、更新操作（同步向量库）未完成（已完成）
6、绑定或解绑知识库操作（未完成）（已完成）
7、绑定知识库的时候，需要对知识库嵌入模型和文档嵌入模型进行校验，如果校验不通过。提醒用户将重新按照知识库的嵌入模型进行嵌入
8、同理：修改文档的嵌入模型的时候也要进行知识库嵌入模型和文档嵌入模型校验，如果校验不通过，提醒用户先解绑知识库再修改嵌入模型
9、同理：修改嵌入模型的时候也要提醒用户将使用新的模型重新嵌入
