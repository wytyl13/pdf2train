...
from pdf2train.api.service.base.minio_service import MinioService # 导入业务层
...
class MinioServer:
    ...
    async def upload_file(
        ...
        try:
            ...
            existing_doc = await self.pdf_document_service.get_by_hash(file_hash)
            if existing_doc:
                return JSONResponse(
                content={
                    "data": {
                        "id": existing_doc['id'],
                        "bucket": existing_doc['bucket_name'],
                        "file_hash": file_hash
                    }
                }
            )
