...
class InstructionBatchExportRequest(BaseModel):
    """按知识库导出微调数据请求"""
    # 支持传单个 int，或者 int 列表 [1, 2, 3]
    kb_id: Union[int, List[int]] = Field(..., description="知识库ID (单个或列表)")
...
class InstructionDatumServer:
    ...
    def register_routes(self, app: FastAPI):
        ...
        router.post("/api/instruction/download_jsonl_by_kb")(self.download_jsonl_by_kb)
    ...
    async def download_jsonl_by_kb(self, request: InstructionBatchExportRequest):
        """
        GET 按知识库下载 JSONL
        支持多选: /api/instruction/download_jsonl_by_kb?kb_id=1&kb_id=2
        """
        try:
            # 1. 调用 Service (传入列表)
            ...
            if isinstance(kb_ids, int):
                kb_ids = [kb_ids]
            data_list = await self.service.export_for_finetuning(doc_id=None, kb_id=kb_ids)
            
            if not data_list:
                return self._response(False, "该知识库下暂无有效的微调数据", code=404)
            ...
            # 3. 构造文件名
            # 如果只有一个 ID，文件名带上该 ID；如果有多个，用 'multi_kb'
            ...
            self.logger.error(f"按KB下载 JSONL 失败: {traceback.format_exc()}")
