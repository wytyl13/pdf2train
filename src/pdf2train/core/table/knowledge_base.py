...
class KnowledgeBase(Base):
    ...
    a_settings = Column("retrieval_settings", JSON, nullable=True, comment="运行时检索策略配置(JSON)")
    # === 4. 权限与生命周期 ===
    ...
    def settings(self) -> RetrievalSettings:
        ...
        """
        if not self.a_settings:
            return RetrievalSettings() # 返回默认配置
        try:
            return RetrievalSettings(**self.a_settings)
        except Exception:
            # 容错处理：防止脏数据导致崩溃
    ...
    def settings(self, config: RetrievalSettings):
        ...
        if isinstance(config, RetrievalSettings):
            self.a_settings = config.model_dump()
        elif isinstance(config, dict):
            self.a_settings = config
        else:
